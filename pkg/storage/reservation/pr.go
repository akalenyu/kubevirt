...
func pvcNameToLabelKey(pvcName string) string {
	hash := sha256.Sum256([]byte(pvcName))
	return v1.PersistentReservationLabelPrefix + hex.EncodeToString(hash[:])[:16]
}

// PersistentReservationPVCLabels returns labels identifying which PVC(s) are
// used with SCSI PersistentReservation by this VMI. Each label key is a
// deterministic hash of the PVC name, allowing pod anti-affinity to prevent
// co-scheduling of VMs sharing the same PR LUN.
func PersistentReservationPVCLabels(vmi *v1.VirtualMachineInstance) map[string]string {
	...
	for _, disk := range vmi.Spec.Domain.Devices.Disks {
		...
		labelKey := pvcNameToLabelKey(pvcName)
		labelValue := pvcName
