		...
		Context("with node selectors", func() {
			...
			It("should add persistent reservation anti-affinity for PR LUN with PVC", func() {
				...
				Expect(pvcCache.Add(&k8sv1.PersistentVolumeClaim{
					...
					ObjectMeta: metav1.ObjectMeta{Namespace: "default", Name: "shared-pvc"},
				})).To(Succeed())
				...
				prLabels := reservation.PersistentReservationPVCLabels(vmi)
				Expect(prLabels).To(HaveLen(1))

				for key, val := range prLabels {
					Expect(pod.Labels).To(HaveKeyWithValue(key, val))
				}
			...
			It("should not add persistent reservation anti-affinity when no PR disks", func() {
				...
				prLabels := reservation.PersistentReservationPVCLabels(&vm)
				Expect(prLabels).To(BeEmpty())
